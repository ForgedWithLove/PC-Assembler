...
def cpu_array
  ...
  Cpu.all.order(socket: :desc, manufact: :desc, model: :desc).each do |x|
    cpu_name = x.manufact + " " + x.model + " (" + x.socket + ", " + x.tdp.to_s + " " + t(:sign_w) + ")"
...
def motherboard_array
  ...
  Motherboard.all.order(socket: :desc, chipset: :desc).each do |x|
    mb_name = x.name + " (" + x.socket + ", " + x.memory_slots.to_s + " " + t(:slots) + ")"
...
def memory_array
  ...
  Memory.all.order(volume_one: :desc, max_freq: :desc).each do |x|
    mem_name = x.volume_one.to_s + " " + t(:sign_gb) + ", " + x.max_freq.to_s + " " + t(:sign_mhz)
...
def psu_array
  ...
  Psu.all.order(wattage: :desc, certificate: :desc).each do |x|
    psu_name = x.wattage.to_s + " " + t(:sign_w) + ", " + x.certificate
...
def disk_array
  ...
  Disk.all.order(storage_type: :desc, volume: :desc).each do |x|
    disk_name = x.storage_type + ", " + x.volume.to_s + " " + t(:sign_tb)
...
def cooler_array
  ...
  Cooler.all.order(socket: :desc, tdp: :desc).each do |x|
    cooler_name = x.socket + ", " + x.tdp.to_s + " " + t(:sign_w)
